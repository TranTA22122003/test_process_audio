...
class AudioToTextRecorder:
    ...
    def __init__(self,
        ...
        if self.enable_realtime_transcription and not self.use_main_model_for_realtime:
            try:
                ...
                # warmup_audio_data, _ = sf.read(warmup_audio_path, dtype="float32")
                warmup_audio_data = np.zeros(16000, dtype=np.float32)
                segments, info = self.realtime_model_type.transcribe(warmup_audio_data, language="vi", beam_size=1)
                model_warmup_transcription = " ".join(segment.text for segment in segments)
            except Exception as e:
    ...
    def transcribe(self):
        ...
        # Lưu bản sao âm thanh vào tệp WAV
        save_audio_to_wav(audio_copy, self.sample_rate)

        # return "Transcribing..." + len(audio_copy).__str__()

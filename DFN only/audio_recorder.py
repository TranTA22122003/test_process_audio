...
class AudioToTextRecorder:
    ...
    def _audio_data_worker(
        ...
        # def _denoise_frame(self, frame_chunks):
        #     """
        #     Takes a list of raw 16kHz audio chunks, denoises them using the DFN
        #     pipeline, and returns a clean 16kHz numpy array.
        #     """
        #     if not self.enable_dfn or not self.dfn_model:
        #         # Nếu DFN không bật, trả về dữ liệu gốc
        #         audio_bytes = b''.join(frame_chunks)
        #         audio_array = np.frombuffer(audio_bytes, dtype=np.int16).astype(np.float32) / INT16_MAX_ABS_VALUE
        #         return audio_array

        #     logger.debug(f"Denoising a frame with {len(frame_chunks)} chunks...")

        #     # Ghép các chunk và chuyển thành mảng 16kHz
        #     original_bytes = b''.join(frame_chunks)
        #     original_audio_16khz = np.frombuffer(original_bytes, dtype=np.int16).astype(np.float32) / INT16_MAX_ABS_VALUE

        #     # Resample LÊN 48kHz
        #     audio_48khz = librosa.resample(original_audio_16khz, orig_sr=self.sample_rate, target_sr=48000)
        ...
        #     # Bỏ chiều batch và chuyển về NumPy
        #     denoised_audio_48khz = denoised_audio_48khz_tensor.squeeze(0).numpy()

        #     # Resample XUỐNG 16kHz
        #     denoised_audio_16khz = librosa.resample(denoised_audio_48khz, orig_sr=48000, target_sr=self.sample_rate)
            
        #     logger.debug("Frame denoised successfully.")
        #     return denoised_audio_16khz
                ...
                try:
                    ...
                    if use_microphone.value:
                        ...
                        # Convert bytes to np.int16
                            ...
                            audio_48k = librosa.resample(audio_chunk.astype(np.float32), orig_sr=16000, target_sr=48000)
                            audio_48k_int16 = (audio_48k * 32767).astype(np.int16) # Scale to int16 range
                            
                            # Shape for mono: (1, num_samples)
                        ...
                        if time_since_last_buffer_message:
                            ...
                            if time_passed > 1:
                                logger.debug("_audio_data_worker writing denoised audio data into queue.")
    ...
    def transcribe(self):
            ...
            If DFN is enabled, it will perform a multi-step denoising process:
            1. Resample the 16kHz audio to 48kHz.
            2. Enhance the audio with the DeepFilterNet model.
            3. Resample the denoised audio back to 16kHz.
            4. Save the denoised audio to a file.
            5. Send the clean audio to Whisper for transcription.

            If DFN is disabled, it saves the original audio and sends it directly
            to Whisper.
            """
            self._set_state("transcribing")
            ...
            if self.enable_dfn and self.dfn_model:
                # --- LUỒNG XỬ LÝ KHI DFN ĐƯỢC BẬT ---
                logger.debug("DFN is enabled. Starting denoising process...")

                original_audio_16khz = audio_copy
                if original_audio_16khz is None or original_audio_16khz.size == 0:
                    logger.warning("No audio data to transcribe.")
                    return ""
                logger.debug(f"Original audio shape (16kHz): {original_audio_16khz.shape}")

                logger.debug("Upsampling audio from 16kHz to 48kHz for DFN...")
                audio_48khz = librosa.resample(original_audio_16khz, orig_sr=self.sample_rate, target_sr=48000)
                
                logger.debug("Converting numpy array to torch tensor for DFN...")
                audio_48khz_tensor = torch.from_numpy(audio_48khz)

                # Thêm chiều batch (batch dimension)
                logger.debug(f"Original tensor shape (1D): {audio_48khz_tensor.shape}")
                audio_48khz_tensor = audio_48khz_tensor.unsqueeze(0)
                logger.debug(f"Reshaped tensor for DFN (2D): {audio_48khz_tensor.shape}")
                
                logger.debug("Enhancing audio with DeepFilterNet model...")
                denoised_audio_48khz_tensor = enhance(self.dfn_model, self.dfn_state, audio_48khz_tensor)
                self.dfn_state.reset()
                
                # Bỏ chiều batch
                logger.debug(f"Denoised tensor shape from DFN (2D): {denoised_audio_48khz_tensor.shape}")
                denoised_audio_48khz_tensor = denoised_audio_48khz_tensor.squeeze(0)
                logger.debug(f"Squeezed tensor back to 1D: {denoised_audio_48khz_tensor.shape}")

                logger.debug("Converting DFN output tensor back to numpy array...")
                ...
                save_audio_to_wav(denoised_audio_16khz, self.sample_rate)

                logger.debug("Sending denoised audio to Whisper for final transcription.")
                return self.perform_final_transcription(denoised_audio_16khz)

            else:
                # --- LUỒNG XỬ LÝ GỐC KHI DFN BỊ TẮT ---
                logger.debug("DFN is disabled or not initialized. Skipping denoising process.")
                
                save_audio_to_wav(audio_copy, self.sample_rate)
            ...
            while self.is_running:
                ...
                if not self.is_recording:
                    ...
                    if self.stop_recording_on_voice_deactivity:
                        ...
                        elif self.enable_silero: # Fallback cho silero nếu webrtc tắt
                            is_speech = self._is_silero_speech(data)

...
import os
import re
from audio_saver import save_audio_to_wav
import gc
from pyrnnoise import RNNoise
import numpy as np
import librosa
from df import enhance, init_df  # DeepFilterNet
...
INIT_BUFFER_CHUNKS = 200  # 200 chunks ~4s buffer như pipeline
INIT_CHUNK_DURATION_MS = 20  # 20ms per chunk
...
class TranscriptionWorker:
    ...
    def run(self):
        ...
        try:
            ...
            warmup_audio_data = np.zeros(16000, dtype=np.float32) # Tạo 1 giây im lặng
            segments, info = model.transcribe(warmup_audio_data, language="vi", beam_size=1)
...
class AudioToTextRecorder:
    ...
    def __init__(self,
                ...
                # NEW: VAD and Noise Reduction Control Flags
                 enable_webrtcVAD: bool = True,
                 enable_silero: bool = True,
                 enable_pyrrnoise: bool = False,
                 ...
                 enable_dfn: bool = True,  # NEW: Bật/tắt DeepFilterNet denoise
                 dfn_model_name: str = "DeepFilterNet2",  # NEW: Model DFN (DeepFilterNet2 là default tốt)
                 dfn_output_dir: str = "denoised_audio",  # NEW: Thư mục lưu file denoised
                 ):
        """
        ...
        """
        self.vad_frame_buffer = collections.deque(maxlen=200)
        ...
        # --- MODIFIED: Store enable flags ---
        self.sample_rate = sample_rate if sample_rate else 48000  # Đổi default thành 48000 để khớp pipeline
        self.buffer_chunks = INIT_BUFFER_CHUNKS
        self.chunk_duration_ms = INIT_CHUNK_DURATION_MS
        self.current_buffer = []
        self.enable_webrtcVAD = enable_webrtcVAD
        self.enable_silero = enable_silero
        self.enable_pyrrnoise = enable_pyrrnoise
        self.enable_dfn = enable_dfn
        self.dfn_output_dir = dfn_output_dir
        if self.enable_dfn:
            try:
                self.dfn_model, self.dfn_state, _ = init_df(model_base_dir=dfn_model_name)
                logger.info(f"DeepFilterNet ({dfn_model_name}) initialized successfully.")
                os.makedirs(self.dfn_output_dir, exist_ok=True)  # Tạo thư mục lưu file
            except Exception as e:
                logger.error(f"Error initializing DeepFilterNet: {e}")
                self.enable_dfn = False  # Tắt nếu lỗi


        if self.level == -1:
            os.environ["FP_LOG_LEVELS"] = "CRITICAL"
            os.environ["OWW_LOG_LEVELS"] = "CRITICAL"

        self.denoiser = None
        if self.enable_pyrrnoise:
        ...
        if self.enable_realtime_transcription and not self.use_main_model_for_realtime:
            try:
                ...
                warmup_audio_data = np.zeros(16000, dtype=np.float32)
    ...
    def _audio_data_worker(
        ...
        def _denoise_frame(self, frame_bytes):
            if not self.enable_dfn or not self.dfn_model:
                return frame_bytes  # nếu DFN tắt thì trả về gốc
            audio_16k = np.frombuffer(frame_bytes, dtype=np.int16).astype(np.float32) / 32768.0
            audio_48k = librosa.resample(audio_16k, orig_sr=16000, target_sr=48000)
            audio_tensor = torch.from_numpy(audio_48k).unsqueeze(0)
            denoised_48k_tensor = enhance(self.dfn_model, self.dfn_state, audio_tensor)
            self.dfn_state.reset()
            denoised_48k = denoised_48k_tensor.squeeze(0).numpy()
            denoised_16k = librosa.resample(denoised_48k, orig_sr=48000, target_sr=16000)
            return (denoised_16k * 32768).astype(np.int16).tobytes()
                ...
                try:
                    ...
                    if use_microphone.value:
                        ...
                        while len(buffer) >= silero_buffer_size:
                            ...
                            # --- Tích hợp RNNoise ở đây ---
                            # Convert bytes to np.int16
                        if denoiser:
                        # Convert bytes to np.int16
                            audio_chunk = np.frombuffer(to_process, dtype=np.int16)
                            if target_sample_rate != 48000:
                                audio_48k = librosa.resample(audio_chunk.astype(np.float32), orig_sr=target_sample_rate, target_sr=48000)
                            else:
                                audio_48k = audio_chunk.astype(np.float32) / 32767  # Normalize
                            audio_48k_int16 = (audio_48k * 32767).astype(np.int16)
                            audio_input = np.expand_dims(audio_48k_int16, axis=0)

                            # Process with RNNoise (denoise_chunk yields per frame)
                            denoised_chunks = []
                            speech_probs = []
                            for speech_prob, denoised_frame in denoiser.denoise_chunk(audio_input):
                                speech_probs.append(speech_prob)
                                denoised_chunks.append(denoised_frame)
                            
                            # Concatenate denoised frames
                            if denoised_chunks:
                                denoised_48k = np.concatenate(denoised_chunks, axis=1).squeeze(0).astype(np.float32) / 32767
                                if target_sample_rate != 48000:
                                    denoised_16k = librosa.resample(denoised_48k, orig_sr=48000, target_sr=target_sample_rate)
                                else:
                                    denoised_16k = denoised_48k  # Nếu sample_rate=48000, giữ nguyên (nhưng sau sẽ resample nếu cần)
                                processed_data_for_queue = (denoised_16k * 32767).astype(np.int16).tobytes()
                        else:
                            processed_data_for_queue = to_process # Fallback nếu lỗi
                    else:
                        # Nếu denoiser bị tắt, dùng dữ liệu gốc
                        processed_data_for_queue = to_process

                        # Feed the denoised data to the audio_queue
                        if time_since_last_buffer_message:
                            # --- NEW: WebRTC VAD ---
                            if hasattr(self_ref, 'webrtc_vad_model'):
                                # chunk ở 16kHz 20ms
                                if self_ref.webrtc_vad_model.is_speech(to_process, target_sample_rate):
                                    self_ref.vad_frame_buffer.append(to_process)

                            # --- Khi đủ 1 frame (200 chunks) ---
                            if len(self_ref.vad_frame_buffer) >= 200:
                                frame_bytes = b''.join(self_ref.vad_frame_buffer)
                                self_ref.vad_frame_buffer.clear()

                                # --- NEW: Denoise ---
                                clean_frame = self_ref._denoise_frame(frame_bytes)

                                # --- NEW: Silero VAD ---
                                audio_np = np.frombuffer(clean_frame, dtype=np.int16).astype(np.float32) / 32768.0
                                audio_tensor = torch.from_numpy(audio_np).unsqueeze(0)
                                speech_prob = self_ref.silero_vad_model(audio_tensor, 16000).item()

                                if speech_prob > self_ref.silero_sensitivity:
                                    audio_queue.put(clean_frame)
                        else:
                            time_since_last_buffer_message = time.time()

                        audio_queue.put(processed_data_for_queue)
    ...
    def transcribe(self):
        ...
        if len(audio_copy) == 0:
            logger.error("Audio buffer is empty. No voice activity detected or recording failed.")
            return ""  # Fallback return empty

        # --- Chuẩn hóa tần số lấy mẫu ---
        if self.sample_rate != 16000:
            logger.debug(f"Resampling full audio from {self.sample_rate}Hz to 16kHz for Whisper...")
            audio_copy = librosa.resample(audio_copy, orig_sr=self.sample_rate, target_sr=16000)

        # --- Nếu có bật DFN thì khử nhiễu ---
        if self.enable_dfn and self.dfn_model:
            logger.debug("DFN is enabled. Starting denoising process...")
            if audio_copy is None or audio_copy.size == 0:
                logger.warning("No audio data to transcribe.")
                return ""

            logger.debug(f"Original audio shape (16kHz): {audio_copy.shape}")
            logger.debug("Upsampling audio from 16kHz to 48kHz for DFN...")
            audio_48khz = librosa.resample(audio_copy, orig_sr=16000, target_sr=48000)

            logger.debug("Converting numpy array to torch tensor for DFN...")
            audio_48khz_tensor = torch.from_numpy(audio_48khz).unsqueeze(0)

            logger.debug(f"Enhancing audio with DeepFilterNet model...")
            denoised_audio_48khz_tensor = enhance(self.dfn_model, self.dfn_state, audio_48khz_tensor)
            self.dfn_state.reset()

            denoised_audio_48khz_tensor = denoised_audio_48khz_tensor.squeeze(0)
            denoised_audio_48khz = denoised_audio_48khz_tensor.numpy()

            logger.debug("Downsampling audio from 48kHz back to 16kHz for Whisper...")
            audio_to_save = librosa.resample(denoised_audio_48khz, orig_sr=48000, target_sr=16000)
        else:
            logger.debug("DFN is disabled or not initialized. Skipping denoising process.")
            audio_to_save = audio_copy

        # --- LƯU FILE WAV ---
        import os
        if not os.path.exists(self.dfn_output_dir):
            os.makedirs(self.dfn_output_dir)

        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        output_filename = os.path.join(self.dfn_output_dir, f"transcribed_{timestamp}.wav")
        try:
            sf.write(output_filename, audio_to_save, 16000)
            logger.info(f"Saved transcribed audio to '{output_filename}'")
        except Exception as e:
            logger.error(f"Failed to save transcribed audio to '{output_filename}': {e}")

        # --- Gửi audio đã xử lý vào mô hình ---
        logger.debug("Sending audio to Whisper for final transcription.")
        return self.perform_final_transcription(audio_to_save)
    ...
    def _recording_worker(self):
        ...
        with buffering and denoising pipeline.
        """
        ...
        try:
            ...
            self.allowed_to_early_transcribe = True
            self.current_buffer = []  # Buffer cho 200 chunks (~4s)
            chunk_samples = int((self.sample_rate / 1000) * self.chunk_duration_ms)  # ~960 samples tại 48kHz, 20ms
            ...
            while self.is_running:
                if last_inner_try_time:
                    ...
                    if last_processing_time > 0.1 and self.use_extended_logging:
                ...
                try:
                    ...
                    if "handle is closed" in str(e):
                        logger.debug("Audio queue handle closed during shutdown")
                        break
                    else:
                        raise
                ...
                if self.handle_buffer_overflow:
                    if self.audio_queue.qsize() > self.allowed_latency_limit:
                        logger.warning("Audio queue size exceeds latency limit. Current size: %d. Discarding old audio chunks.", self.audio_queue.qsize())
                        while self.audio_queue.qsize() > self.allowed_latency_limit:
                            self.audio_queue.get()

                # Feed the extracted data to the current_buffer
                if time_since_last_buffer_message:
                    time_passed = time.time() - time_since_last_buffer_message
                    if time_passed > 1 and self.use_extended_logging:
                ...
                # Buffer chunk và xử lý WebRTC VAD trên chunk nhỏ
                self.current_buffer.append(data)
                chunk_audio = np.frombuffer(data, dtype=np.int16).astype(np.float32) / INT16_MAX_ABS_VALUE
                if self.enable_webrtcVAD and self.webrtc_vad_model:
                    if self.sample_rate != 16000:
                        chunk_16khz = librosa.resample(chunk_audio, orig_sr=self.sample_rate, target_sr=16000)
                        chunk_data_16khz = (chunk_16khz * INT16_MAX_ABS_VALUE).astype(np.int16).tobytes()
                    else:
                        chunk_data_16khz = data
                    is_webrtc_active = self._is_webrtc_speech(chunk_data_16khz)

                # Khi buffer đầy (200 chunks), xử lý frame lớn
                if len(self.current_buffer) >= self.buffer_chunks:
                    frame_data = b''.join(self.current_buffer)
                    self.current_buffer = []  # Reset buffer

                    # Denoise với DFN nếu enable
                    if self.enable_dfn and self.dfn_model:
                        logger.debug("Denoising buffered frame with DFN...")
                        frame_audio = np.frombuffer(frame_data, dtype=np.int16).astype(np.float32) / INT16_MAX_ABS_VALUE
                        if self.sample_rate != 48000:
                            frame_48khz = librosa.resample(frame_audio, orig_sr=self.sample_rate, target_sr=48000)
                        else:
                            frame_48khz = frame_audio
                        frame_48khz_tensor = torch.from_numpy(frame_48khz).unsqueeze(0)
                        denoised_48khz_tensor = enhance(self.dfn_model, self.dfn_state, frame_48khz_tensor)
                        self.dfn_state.reset()
                        denoised_48khz = denoised_48khz_tensor.squeeze(0).numpy()
                        if self.sample_rate != 48000:
                            denoised_audio = librosa.resample(denoised_48khz, orig_sr=48000, target_sr=self.sample_rate)
                        else:
                            denoised_audio = denoised_48khz
                        frame_data = (denoised_audio * INT16_MAX_ABS_VALUE).astype(np.int16).tobytes()
                        logger.debug("Frame denoised successfully.")

                    # Áp dụng Silero VAD trên frame đã denoised, chia thành các đoạn 512 mẫu
                    if self.enable_silero and self.silero_vad_model:
                        if self.sample_rate != 16000:
                            frame_audio = np.frombuffer(frame_data, dtype=np.int16).astype(np.float32) / INT16_MAX_ABS_VALUE
                            frame_16khz = librosa.resample(frame_audio, orig_sr=self.sample_rate, target_sr=16000)
                            frame_data_for_silero = (frame_16khz * INT16_MAX_ABS_VALUE).astype(np.int16).tobytes()
                        else:
                            frame_data_for_silero = frame_data

                        # Chuyển frame_data_for_silero thành mảng numpy
                        audio_data = np.frombuffer(frame_data_for_silero, dtype=np.int16)
                        total_samples = len(audio_data)

                        # Chia thành các đoạn 512 mẫu
                        for i in range(0, total_samples, 512):
                            segment = audio_data[i:i + 512]
                            if len(segment) == 512:  # Đảm bảo đoạn có đúng 512 mẫu
                                segment_bytes = segment.tobytes()
                                is_silero_active = self._is_silero_speech(segment_bytes)
                                if is_silero_active:
                                    # Thêm đoạn có giọng nói vào frames
                                    start_idx = i * 2  # Vì mỗi mẫu là 2 byte (int16)
                                    end_idx = start_idx + 1024  # 512 mẫu × 2 byte
                                    self.frames.append(frame_data[start_idx:end_idx])

                    # Logic không recording
                    if not self.is_recording:
                        ...
                        if wake_word_activation_delay_passed and not delay_was_passed and self.use_wake_words and self.wake_word_activation_delay:
                    ...
                    # Logic khi recording
                        ...
                        if self.stop_recording_on_voice_deactivity:
                            is_speech = False
                            if self.silero_deactivity_detection and self.enable_silero:
                                is_speech = is_silero_active  # Sử dụng kết quả từ frame lớn
                            elif self.enable_webrtcVAD:
                                is_speech = self._is_webrtc_speech(data, True)
                            elif self.enable_silero:
                                is_speech = is_silero_active
                            ...
                            if not is_speech:
                                if self.speech_end_silence_start == 0 and (time.time() - self.recording_start_time > self.min_length_of_recording):
                                    ...
                                    (time.time() - self.speech_end_silence_start > self.early_transcription_on_silence) and self.allowed_to_early_transcribe:
                                        self.transcribe_count += 1
                                ...
                                if self.speech_end_silence_start and time.time() - self.speech_end_silence_start >= self.post_speech_silence_duration:
                                    ...
                                    if self.use_extended_logging:
                                        logger.info(f"voice deactivity detected at {silence_start_time}, time since silence start: {time_diff:.3f} seconds")
            ...
            # Đảm bảo dọn dẹp tài nguyên nếu cần
            if self.use_extended_logging:
                logger.debug('Debug: Exiting _recording_worker method with cleanup')
    ...
    def _is_silero_speech(self, chunk):
        ...
        """
        if not self.enable_silero or not self.silero_vad_model:
            return False
    ...
    def _is_webrtc_speech(self, chunk, all_frames_must_be_true=False):
        ...
        Args:
            ...
            16000 sample rate and 16 bits per sample)
        """
        if not self.enable_webrtcVAD or not self.webrtc_vad_model:
            return False
        else:
    ...
    def _is_voice_active(self):
            """
            Determine if voice is active based on enabled VADs.
            """
            # Nếu không có VAD nào được bật, coi như luôn có giọng nói để không bị kẹt
            if not self.enable_webrtcVAD and not self.enable_silero:
                return True

            active_checks = []
            if self.enable_webrtcVAD:
                active_checks.append(self.is_webrtc_speech_active)
            if self.enable_silero:
                active_checks.append(self.is_silero_speech_active)
            
            # Logic AND: tất cả các VAD được bật đều phải phát hiện giọng nói
            return all(active_checks)
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        try:
            ...
            # Đảm bảo tài nguyên được giải phóng ngay cả khi có lỗi

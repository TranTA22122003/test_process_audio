...
class AudioToTextRecorder:
    ...
    def __init__(self,
        ...
        self.buffer_chunks = 200
        self.vad_audio_buffer = collections.deque(maxlen=self.buffer_chunks)
    ...
    def transcribe(self):
        """
        Điều phối việc xử lý, lưu và gửi âm thanh đi phiên mã.
        Logic xử lý và lưu file được ủy quyền cho module audio_saver.
        """
        self._set_state("transcribing")
        audio_copy = copy.deepcopy(self.audio) # Lấy toàn bộ buffer âm thanh đã ghi

        if self.enable_dfn and self.dfn_model:
            # --- LUỒNG XỬ LÝ KHI DFN ĐƯỢC BẬT (TƯƠNG ỨNG PIPELINE) ---
            ...
            # BƯỚC 1: Resample LÊN 48kHz cho Denoise
            logger.debug("Upsampling audio from 16kHz to 48kHz for DFN...")
            audio_48khz = librosa.resample(original_audio_16khz, orig_sr=self.sample_rate, target_sr=48000)
            
            # BƯỚC 2: Khử nhiễu với DFN
            ...
            # BƯỚC 3: Resample XUỐNG 16kHz cho Whisper
            logger.debug("Downsampling audio from 48kHz back to 16kHz for Whisper...")
            denoised_audio_16khz = librosa.resample(denoised_audio_48khz, orig_sr=48000, target_sr=self.sample_rate)

            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            # THÊM DÒNG NÀY ĐỂ LƯU FILE ÂM THANH ĐÃ KHỬ NHIỄU
            save_audio_to_wav(denoised_audio_16khz, self.sample_rate)
            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

            # BƯỚC 4: Gửi âm thanh SẠCH đến bước cuối cùng (Whisper)
            ...
            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            # THÊM DÒNG NÀY ĐỂ LƯU FILE ÂM THANH GỐC
            ...
            while self.is_running:
                ...
                if not self.is_recording:
                    ...
                    if self.stop_recording_on_voice_deactivity:
                        ...
                        is_speech = False
                          # --- THAY ĐỔI TẠI ĐÂY ---
                          # Ưu tiên Silero VAD để quyết định dừng ghi âm vì nó chính xác hơn
                          # Đây là bước "bộ lọc tinh" của pipeline.
                        if self.silero_deactivity_detection and self.enable_silero:
                              is_speech = self._is_silero_speech(data)
                          
                          # Nếu Silero không được dùng để phát hiện điểm dừng, mới dùng đến WebRTC
    ...
    def _is_voice_active(self):
                ...
                # --- THAY ĐỔI TẠI ĐÂY ---
                # Logic OR: Chỉ cần MỘT VAD phát hiện giọng nói là đủ để bắt đầu.
                # Điều này cho phép WebRTC VAD phản ứng nhanh để bắt đầu ghi âm.
